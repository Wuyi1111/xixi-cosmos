...
│   │   ├── StarView.jsx           # 🎨⚙️ 「归星」tab：精简个人信息 + 白噪音 + 睡前仪式 + 数据统计
...
- `git rm src/views/MineView.jsx`（222 行，已被 StarView 取代）
- `git rm src/widgets/DreamCard.jsx`（270 行，TonightView 重构后无引用）
- `git rm src/widgets/BreathingWidget.jsx`（423 行，StarView/SplashScreen 自带呼吸动画）
- WishPoolView docstring 顺手改：入口描述从 MineView 改为 StarView
- README 项目结构图同步移除这 3 个文件的条目
...
- TreeholeView `hotTasks` 现在合并两部分：
  1. 用户自己公开发布的明日约定（`userData.userChallenges`），排在最前
  2. 系统推荐（`displayedSuggestions`），跟随之前一样
- 自己发布的卡片右上角加 emerald 色「你发布的」徽章
- 自己发布的卡片：不显示「换一条」按钮（不能刷掉自己的）+ 不显示「跟随」按钮，
  改为静态「已发布」徽章（不能跟随自己）
- 跟随人数：用户挑战取真实 `followers.length`；系统推荐继续用伪随机
- 之前发布"公开"明日约定后没有任何反馈，现在能在热门任务列表立即看到
