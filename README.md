...
**当前版本：v4.6.2** · [在线访问 →](https://wuyi1111.github.io/xixi-cosmos/)
...
### v4.6.2 · 2026-05-19 — 底部导航放大 + 减"下巴"感

- 4 个 tab 图标从 22px 改为 28px，激活态描边粗细微调
- 标签字号 `text-[10px]` 改 `text-xs`（移动端 @media 下 → 14px）
- 图标容器内边距 p-2 → p-2.5（激活态背景胶囊更大更柔），rounded-xl → rounded-2xl
- TabButton 宽度 w-16 → w-20，与放大后的图标更协调
- 图标 + 标签的纵向间距 space-y-1 → gap-1.5
- nav 内层高度 h-20 (80px) → h-[5.5rem] (88px)，给放大后的内容更多呼吸
- main 底部 padding 同步 6rem → 6.5rem，确保内容不被新 nav 盖住
...
按 semver（major.minor.patch）但放宽尺度。三档升号 + 操作清单 + 边界判定。

### 三档升号

| 改动 | 升号 | 实际发过的例子 |
|---|---|---|
| 修 bug · 小改 · 文案 · 视觉微调 · 纯重构 | **patch +1** | v4.5.0 → v4.5.1（打卡卡片视觉 + 文案）<br>v4.6.1 → v4.6.2（导航条放大）<br>v4.3.0 → v4.3.1（App.jsx 拆分）|
| 新功能 · 多个改动合并 · 引入 userData 新字段 | **minor +1** | v4.3.x → v4.4.0（新增星愿池）<br>v4.4.x → v4.5.0（重组微澜 + 新 tab）<br>v4.5.x → v4.6.0（粒子动效 + 设置下拉栏）|
| 破坏性变更（数据不兼容 / 删功能 / 改架构）| **major +1** | 还没发过；**会先确认再升** |

### 每次发版本必做的 5 件事

1. 改 [package.json](package.json) 的 `version` 字段
2. `npm install --package-lock-only` 同步 lock 文件
3. `npm run build` 本地先验证编译通过
4. commit message 用 `release: vX.Y.Z — 描述` 格式
5. 本 README 同步两处：
   - 顶部 `**当前版本：vX.Y.Z**`
   - 版本日志加一条新 entry（含 commit 日期 + 改动 bullet）

push 后 vite 插件会在 `dist/version.json` 写入新 buildTime，App 内的"检查更新"按钮会通过对比这个文件提示用户。

### 怎么判定"功能"还是"小改"

凭"用户能不能感知"切分：

- **patch** — 用户看不出来 / 看出来但是"哦哦"的程度
  - 改个文案、调个间距、修个 bug、换个颜色
  - 也包括纯工程改动（重构、拆文件、加注释）

- **minor** — 用户能感知到的新东西
  - 多了一个 tab / 一个页面 / 一个交互
  - 改了主要的视觉体系（比如导航全变下拉栏）
  - userData 加了新字段（哪怕只是一个数字）

- **major** — 会让老数据 / 老链接 / 老习惯失效的改动
  - 比如删了某个 tab、把存数据格式整个换了
  - 这种我会**先停下来跟你确认**再升

### 边界案例参考

- "星愿池" 加了新页面但没接后端 → **minor**（功能存在了，哪怕是 mock）
- "明日"加完成按钮 + 计数（v4.5.2）→ **patch**（只是给已有 tab 加点东西；
  虽然加了 userData 字段，但属于增量补丁性质，按 patch 处理）
- 设置面板全部下拉栏 + 打卡粒子化（v4.6.0）→ **minor**（两个明显新东西合一）
- App.jsx 拆 14 个文件（v4.3.1）→ **patch**（纯重构，用户行为零变化）

### 不需要升号的情况

- 只改 README / 注释（纯文档）
- 你直接在 GitHub 网页改了某行（我之后跟着走，不主动升号）

---

如果哪天想严格走 semver、或者想改成日历版本号（如 `2026.05.19.1`），告诉我。当前规则跑得顺、改动密度撑得住，不主动改。
